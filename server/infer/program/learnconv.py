...
if __name__ == "__main__":
    ...
    if sys.argv[1] == 'don':
        net.train(songs=songs, minibatch=minibatch, valsong=valsong,
              epochs=epochs, device=device, soundlen=soundlen, saveplace=modelpth+'don', logplace='./convoutonsetdon.txt', usedonka=1)
    if sys.argv[1] == 'ka':
        net.train(songs=songs, minibatch=minibatch, valsong=valsong,
              epochs=epochs, device=device, soundlen=soundlen, saveplace=modelpth+'ka', logplace='./convoutonsetka.txt', usedonka=2)
    if sys.argv[1] == 'both':
        net.train(songs=songs, minibatch=minibatch, valsong=valsong,
